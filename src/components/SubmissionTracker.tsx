...
// Organic wait estimate: fixed-seed accumulation so position N always > position N-1
// Uses a constant seed so every caller gets the same ladder of times.
function estimateWait(position: number): string {
  // Deterministic pseudo-random per slot index (fixed seed = 42)
  const rng = (i: number) => { const x = Math.sin(42 + i * 127.1) * 10000; return x - Math.floor(x); };
  let totalMin = 0;
  for (let i = 0; i < position; i++) {
    // Each slot adds 3-7 min
    totalMin += 3 + Math.floor(rng(i) * 5);
  }
  // Nudge away from "too round" numbers
  if (totalMin % 5 === 0) totalMin += 1;
  if (totalMin % 10 === 0) totalMin += 2;
