...
export function SubmissionForm({ watchlistRef, streamerId, streamerSlug, onSubmissionTracked }: SubmissionFormProps) {
  ...
  const rawRequireArtist = showArtist && getRequired('artist_name', false);
  const rawRequireTitle = showTitle && getRequired('song_title', false);
  // If both are individually required, treat as "at least one" instead of both
  const bothRequired = rawRequireArtist && rawRequireTitle;
  const requireArtist = bothRequired ? false : rawRequireArtist;
  const requireTitle = bothRequired ? false : rawRequireTitle;
            ...
            if (data.hashedToken || data.actionLink) {
              try {
                ...
                if (tokenHash) {
                  ...
                  if (!otpError) {
                    console.log('[AUTO-LOGIN] User signed in after payment');
                  } else {
          ...
          >
            ...
            <div className="space-y-3">
              ...
              {showMessage && (
                <div>
                  <label className="text-xs text-muted-foreground mb-1.5 block">
                    {messageLabel} {requireMessage ? <span className="text-destructive">*</span> : <span className="text-muted-foreground/60 text-xs">({t('submission.optional')})</span>}
                  </label>
                  <Textarea
