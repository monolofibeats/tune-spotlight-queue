...
export function WidgetWrapper({ widgetId, isEditing, children, onRemove, isPoppedOut, textScale = 100 }: WidgetWrapperProps) {
  ...
  return (
    <div className={`h-full flex flex-col overflow-hidden ${
      isEditing 
        ? 'rounded-xl ring-1 ring-primary/30 ring-dashed bg-card/30 border border-border/30 transition-all' 
        : 'rounded-none bg-card/40 border-r border-b border-border/30'
      ...
      <div className="flex-1 min-h-0 min-w-0 overflow-auto widget-container" style={{ containerType: 'size', ...zoomStyle }}>
        {children}
      </div>
    </div>
  );
